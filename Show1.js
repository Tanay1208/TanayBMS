...
export default function Show1() {
    ...
    const [obj, setObj] = useState({ selected: 0, label: "A1", show: 1 });
    ...
    useEffect(() => {
        setLoading(true);
        let already= localStorage.getItem("show 1")
    ...
    //  }
        ...
        if(data ? data[1] : 0) {
            ...
            let obj2Temp = [];
            for (let i = 0; i < 6; i++) {
                    obj2Temp[i] = { selected: 0, label: `B${i + 1}`, amount: "280" };
            }
            setObj2(obj2Temp);
        }
       ...
       { !loading &&<div>
            ...
            <div>
                ...
                <Button onClick={()=>{
                    ...
                    localStorage.setItem("Show Details","Show1" );
                    let getData = localStorage.getItem('show 1');
                    localStorage.setItem("show 1",JSON.stringify([obj1,obj2, obj3]))
                    // let fixedData = [];
                    // if(setData){
